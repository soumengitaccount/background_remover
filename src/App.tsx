...
import { removeBackgroundAction, proxyImageAction } from "./actions";
...
export default function App() {
  ...
  // Run AI Background Removal (Server-side powered with client-side fallback)
  const runBackgroundRemoval = async (imageSource: File | Blob) => {
    ...
    try {
      ...
      let data;
      try {
        // Execute the native React Server Action
        data = await removeBackgroundAction(base64Payload, imageSource.type || mimeType || "image/png");
      } catch (serverError) {
        console.warn("Server-side background removal failed or unavailable, falling back to local browser AI...", serverError);
        setProgress({
          status: "processing",
          percent: 50,
          message: "Server busy. Loading local browser AI engine (downloads models on first run)...",
        });
        ...
        const resultBlob = await imglyRemoveBackground(imageSource, {
          progress: (status: string, val: number) => {
            ...
            setProgress({
              status: "processing",
              percent: Math.min(95, Math.round(50 + (displayPercent * 0.45))),
  ...
  const handleSelectPreset = async (preset: PresetImage) => {
    ...
    try {
      // Execute the native React Server Action to proxy the external image
      const base64 = await proxyImageAction(preset.url);
      const proxyUrl = `/api/proxy-image?url=${encodeURIComponent(preset.url)}`;
      setOriginalUrl(proxyUrl);
      setOriginalBase64(base64);
      
      // Convert base64 to Blob for the model
